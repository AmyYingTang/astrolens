...
export async function renderAnnotated(opts: RenderAnnotatedOptions): Promise<void> {
  const svg = Buffer.from(buildOverlaySvg(opts.report));
  let pipeline = sharp(opts.imagePath).composite([{ input: svg, top: 0, left: 0 }]);
  pipeline = opts.outPath.toLowerCase().endsWith('.png')
    ? pipeline.png()
    : pipeline.jpeg({ quality: opts.quality ?? 92 });
